...
class GeneralSatelliteTasking(Env, Generic[SatObs, SatAct]):
    ...
    def _get_info(self) -> dict[str, Any]:
        ...
        total_processed = sum(
            ...
            for sat in self.satellites 
            if hasattr(sat, 'processed_data_total')
        )
        total_offloaded = sum(
            sat.offloaded_data_total 
            for sat in self.satellites 
        ...
        # 最终安全检查：确保 total_energy 不是 NaN/Inf
    ...
    def _step(self, actions: MultiSatAct) -> None:
        ...
        if hasattr(self.scenario, 'process_task_arrivals'):
            ...
            if arrivals > 0:
                logger.info(f"Poisson arrival: {arrivals} tasks arrived this step")
        ...
        for satellite, action in zip(self.satellites, actions):
            ...
            if not is_no_action(action):
                ...
                # 填充邻居卫星列表（用于协作卸载）
                # 通过 action_builder.action_spec 访问动作实例（不是 actions）
                if hasattr(satellite, 'action_builder') and hasattr(satellite.action_builder, 'action_spec'):
